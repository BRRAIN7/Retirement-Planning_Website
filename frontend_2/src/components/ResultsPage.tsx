...
const ResultsPage = ({
  ...
  useEffect(() => {
    if (results) {
      ...
      // 2. Calculate Monthly Contribution dynamically from Form Data
      // (Annual Income / 12) - Monthly Expenses
      const formData = results.formData || {}; // Safety check
      ...
      const calculatedMonthlyContribution = (annualIncome / 12) - monthlyExpenses;
      // Ensure we don't show negative contribution for projection purposes
      const monthlyContribution = Math.max(0, calculatedMonthlyContribution);

      const r = 0.06; // Assumed annual return rate
      const n = yearsToRetirement;
      let projectedAtRetirement = currentAssets;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-primary/5 to-accent/5 py-12">
      <div className="container mx-auto max-w-6xl px-4">
        ...
        {/* 🧾 Summary Cards - CHANGED TO 3 COLUMNS */}
